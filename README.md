...
一款轻量级 Chrome 浏览器代理管理扩展，通过直连本地 Clash Verge / Mihomo 的 RESTful API，实现浏览器级别的代理控制、节点切换与状态监控。

> 本扩展不替代 Clash Verge，而是作为浏览器端的补充控制面板。订阅管理、规则配置、内核更新等仍由 Clash Verge 负责。
...
- **节点切换** — 读取 Clash 配置的代理组，支持 Selector / URLTest / Fallback 类型，子组可点击跳转
...
- 流量数据通过 SSE 直连 mihomo `/traffic` 端点
- IP 检测从 Popup 直接发起请求，经过 Chrome Proxy API，反映真实出口 IP
